...
include("Utils.jl")
using  Gnuplot, BenchmarkTools, LinearAlgebra, Plots, Images, Colors, PyCall, Distributions, StatsBase, Parameters
# using ..Utils: Utils, InputParameters, InputFiles, unpack_selected, unpack_selected_filenames
using ..Utils

export ImportLattices
export CalcSample
...
# const len_vector = Nx*Ny*Nz

"""
Imports lattices from the specified files
In: Parameter-struct, filename-struct
Out: z-component of velocities in entire domain, vector with labels used by LBPM
"""
function ImportLattices(params::InputParameters, input_files::InputFiles)
    local Nx,Ny,Nz,len_vector = unpack_selected(params,:Nx,:Ny,:Nz,:len_vector)
    infile_raw, infile_velocities = unpack_selected_filenames(input_files,:infile_raw,:input_files_velocities)
    data_infile_raw = Vector{UInt8}(undef,len_vector)
    read!(infile_raw, data_infile_raw)
    ...
    db = silo.SiloFile($infile_velocities, create=False, mode=silo.DB_READ)
    ...
    # "Reshape data (might not be needed other than for plotting) "
    ...
    return vec(data_velocities), raw_vector
end
...
"""
Calculate the transversal area distributions in a single sample
In:
Out: ap_dv,aw_dv,an_dv, [solid,w,nw]
"""
function CalcSample(params::InputParameters, data_velocities::Vector{Float64},raw_vector::Vector{Int8}, velocity_increments::Vector{Float64})

# Sort velocities in entire array
voxel_length,Nx,Ny,Nz,L,dv,len_vector = unpack_selected(params,:voxel_length, :Nx,:Ny,:Nz,:L,:dv,:len_vector)
...
for j in 1:Nz # Loop over layers
   ...
   vel_hist_all[:,j] = vel_set # Store the weights in array
    ...
    # Calculate saturations in entire sample
    solid=countmap(raw_vector)[0] # Number of vozels of solid, nw and w type.
    nw=countmap(raw_vector)[2]; nw_frac=nw/solid
    w=countmap(raw_vector)[1]; w_frac=w/solid
    porosity= (w+nw)/solid
    saturations=[porosity,w_frac, nw_frac]
    return ap_dv,aw_dv,an_dv, saturations
end # CalcSingleLayer
